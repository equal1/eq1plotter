...
###################################################################
    ...
    def __init__( self ):
      ...
      txt = '$Revision: 1.156 $'                   # sos revision number, auto assigned by sos when doing a checkin
...
#                     rated_power_values_idnum_list[ par ].append( prd_test['#'])
                     ...
                     if par not in rated_power_values:
                        rated_power_values[ par ] = [ None ] * 4

                     # save the rpwr value for this specific test in the rated_power_values dict
                     # (this is a 4 element list, one power level for each subband)
                     sb = prd_test[ 'sub-band' ]

                     for i in [0,1,2,3]:
                         if self.sub_band_list[i] == sb :
                              if  rated_power_values[ par ][i] != None and abs( float( rpwr ) - rated_power_values[ par ][i]) > 0.01 :
                              ...
                              rated_power_values[ par ][i] = float( rpwr )

                     if sb == self.sub_band_list[4] :
                         if rated_power_values[ par ][0] != None : rated_power_values[ par ][0] = float( rpwr )
                         if rated_power_values[ par ][1] != None : rated_power_values[ par ][1] = float( rpwr )
                     if sb == self.sub_band_list[5] :
                         if rated_power_values[ par ][2] != None : rated_power_values[ par ][2] = float( rpwr )
                         if rated_power_values[ par ][3] != None : rated_power_values[ par ][3] = float( rpwr )

                     # save away the actual rated power, we will use this later when making up the graph title
                     prd_test[ 'rated power value'] = rpwr
...
#               print '@@@@(add_logfile) ', linecount, self.datacount, section_data[ 'section' ], ln, lnf, lnll, nnll
            ...
            if atr_column_header == True                                   or \
               ( logfile_type == 'excel' and done_column_header == False and not re.match( r'^[\-\+0-9]', row[0] )) :    # look for excel column name row


               atr_column_header = True
               if logfile_type != 'excel':
                  atr_logfile = 'atr'
                  done_column_header = True
               else:
...
#            print logfile_type
            ...
            if ( logfile_type == 'atr'   and first_field_is_number  and done_column_header == True ) or \
...
#                print ''
            ...
            if logfile_type == 'excel' and done_column_header_pre == True:
                  done_column_header     = True
                  done_column_header_pre == False
          ...
          print '   .read %s %6d lines, read in records %6d to %6d' % (logfile_type, linecount, rn_start, self.datacount)
      ...
      self.add_missing_columns( rn_start, self.datacount, logfile_type )
...
#                          raise Exception
    ...
    def add_missing_columns(self, rn_start=None, rn_finish=None, logfile_type=None ):
...
#        print '(add_missing_columns) doing amam ', vam_name, rn_start, rn_finish
...
#         self.add_new_column( 'AMAM conversion Voutpk/VAM (dV/dV)' )
         ...
         for rn in range( rn_start, rn_finish ):
#                print '(add_missing_columns)               rn=%s vam_name=%s line=%s  step=%s vam=%s' % (rn, vam_name, self.data[ 'linenumber' ][rn], self.data[ 'Step' ][rn], self.data[ vam_name ][rn])
                 try:
...
#         self.scale_time_data( '[Time] VBAT_THIRD',   '[Time] VBAT*'    , 3,  rn_start, rn_finish )
      ...
      # Add the Ref Pout data and the @ rated_power data.
      # If we are doing a VRAMP SEARCH we will get the nominal condtions from the
      # vramp search conditions. If no VRAMP SEARCH data is present then we will get
      # only 4 nominal conditions (one for each sub-band)

#      print '(add_missing_column) checking for vramp search'

      if 'Pout(dBm)' in self.data and '[csv] Script File Name' not in self.data and logfile_type == 'atr':
...
#             print '(add_missing_column) found vramp search'

             # Get a list of vramp search nominal conditions, each specifiying
             # vbat, freq, pin. Also for each nom condition, specify the range of records that it applies
             ...
             if rnl_list != None and len( rnl_list ) > 0 :
                 for rnl in rnl_list:
                     self.update_nom_conditions( rnl[0] )
                     self.add_cal_pout_data( rnl[1], rnl[2], testname='Output Power & Efficiency', matchname='Vramp Voltage')
                     self.add_cal_pout_data( rnl[1], rnl[2], testname='AM-AM AM-PM Distortion'   , matchname='Step')

         else:
             rnl_list = self.get_nom_conditions( rn_start, rn_finish)
             if rnl_list != None and len( rnl_list ) > 0 :
                 self.update_nom_conditions( rnl_list[0][0] )
                 self.add_cal_pout_data( rn_start, rn_finish, testname='Output Power & Efficiency', matchname='Vramp Voltage')
                 self.add_cal_pout_data( rn_start, rn_finish, testname='AM-AM AM-PM Distortion'   , matchname='Step')
...
#              print '    (get_nom_conditions)  nom_freq_list ', nom_freq_list, rns, rnf
    ...
    def get_nom_conditions_obsolete( self, rn_start, rn_finish ):
      ...
      del  self.values_dict[ 'Freq(MHz)']
      self.add_values_dict('Freq(MHz)', rn_start,rn_finish )
...
#                print '(add_cal)             ... trying', f, i, fmin_diff_i, ' inband_freq_count=', inband_freq_count, fdiff[i],  fdiff[ fmin_diff_i ]
           ...
           self.nom_freq_list.append( nom_freq )
...
#       print '(add_cal_pout_data)     %10s  target=%10s found=%10s' % ( n, self.nom_target_value_list[ix], nom_found_value_list[ix] )
      ...
      print '    (get_nom_conditions)  nom_freq_list ', self.nom_freq_list, rn_start, rn_finish

      # redo the values_dict for freq because we corrupted it earlier
      del  self.values_dict[ 'Freq(MHz)']
      self.add_values_dict('Freq(MHz)' )
...
#                                ]
    ...
    ###################################################################
    def add_cal_pout_data( self, rn_start, rn_finish, testname='Output Power & Efficiency', matchname='Vramp Voltage' ):
...
#                sel_values = 'LIST TOO LONG'
                sel_values = '['
                for sv in sel_lst[:5]:
                    sel_values = '%s %0.3f,' % (sel_values , sv )
