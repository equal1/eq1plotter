...
import re , os, getopt, math, commands, csv, types, glob, time, shutil, getpass, subprocess, traceback
...
###################################################################
    ...
    def __init__( self ):
      ...
      txt = '$Revision: 1.251 $'                   # sos revision number, auto assigned by sos when doing a checkin
...
#            fip = open(logfilename, "rb")
          ...
          if self.logfile_extension.lower() in ['.s1p', '.s2p'] :
          ...
          for row in reader:
            ...
            if self.logfile_type in ['s1p','s2p'] :
                 if self.s2p_type == None:
                     if re.search(r'^#\s+Hz\s+S\s+MA\s+R', row[0]): 
                        self.s2p_type = 'mag_phi'
                        do_s2p_subsample = self.s2p_subsample
                     elif re.search(r'^#\s+Hz\s+S\s+RI\s+R', row[0]): 
                        self.s2p_type = 'real_imag'
                     elif re.search(r'^#\s+Hz\s+S\s+dB\s+R', row[0]): # Hz S  dB   R 50
...
#           print '(add_logfile) looking for  &&', linecount, row[0]
            ...
            if ( self.logfile_type == 'atr'   and first_field_is_number  and done_column_header == True ) or \
               ...
               ( self.logfile_type in ['s1p', 's2p']   and done_column_header == True ):
...
#                print ''
      ...
      print r' (Loading Time %0.2f/%0.2f sec)' % ( time.time()-time_read, time.time()-time_start)
...
#                  print '(add_logfile) vramp data ',self.data['Vramp File'][rn], [ vramp_release, hb_lb, seg, voltage, full_filespec, pcl_level ]
      ...
      if 'Frequency' in self.data :
           ...
           for rn in range( rn_start, rn_finish ):
              if (self.data['Frequency'][rn]) not in UNDEFS:                               # tests if there is data in 'Freq(s2p)' log file columnPortOut(dBm)
                self.data['Freq(MHz)'][rn] = self.data['Frequency'][rn]
                self.data['Freq(GHz)'][rn] = self.data['Frequency'][rn] / 1e3
      ...
      if 'Test Freq(MHz)' in self.data:
           self.add_new_column('Freq(GHz)')
           for rn in range( rn_start, rn_finish ):
              if 'Freq(s2p)' not in self.data or self.data['Freq(s2p)'][rn] in UNDEFS:
...
##  To get S11 and S22 in VSWR, need to use (1+ mag (S11, or 22))/(1- mag (S11, or 22)),   email from H.Sun 12nov09
      ...
      for S in ['s11', 's21', 's12', 's22']:
         ...
         if S_mag in self.data and S_phi in self.data:
             ...
             if S == 's11':
               self.add_new_column('s11_Zreal(ohm)')
               self.add_new_column('s11_Zimag(ohm)')
               for rn in range( rn_start, rn_finish ):  
                   ...
                   s11 = complex(r,i)
                   Z = 50 * (1 + s11)/(1 - s11)
                   self.data['s11_Zreal(ohm)' ][rn] = Z.real
                   self.data['s11_Zimag(ohm)' ][rn] = Z.imag
...
#      self.add_linear_gain(rn_start, rn_finish )
    ...
    ######################################################################################
    def s2p_to_touchstone_lines(self, rn):
    ...
    def add_all_power_values( self, rn_start, rn_finish ):
      ...
      if 'Adj Pwr Out(dBm)' in self.data or 'PSA Pwr Out(dBm)' in self.data or 'Pout(dBm)' in self.data or 'Pout_pm(dBm)' in self.data:
         ...
         for rn in range( rn_start, rn_finish ):
             ...
             if    'Pout_pm(dBm)' in self.data and self.data[ 'Pout_pm(dBm)' ][rn] != None:
...
#                     self.add_ref_pout_data( rnl, rn_start, rn_finish, testname='Power_and_Eff',             matchname='Vramp Voltage')
    ...
    ######################################################################################
    def copy_column_data( self, name1, name2, rn_start, rn_finish ):
    ...
    def add_ref_pout_data_pypat( self, rn_start, rn_finish, matchname ):
         ...
         for rn in range( rn_start, rn_finish ):
            ...
            if not ( testname in ['Output Power & Efficiency','Edge ORFS Modulation'] or 'search' in testname.lower() ): 
                continue
...
#         if re.search( r'Pout', name , re.I) or re.search( r'VSWR', name , re.I):
       ...
       if xname != 'Frequency of Spur (MHz)' and xname[:5] != '[Time]' :
         ...
         if 'Modulation' in self.data and  \
            ( self.data['Modulation'][first_rn] in ['wcdma','hsupa','hsdpa','edge','tdscdma', 'EDGE-Option'] or \
         ...
         for sc in self.value_dict_names:
            if sc not in xynames and sc in self.series_seperated_list and sc != 'Vramp Voltage':
               if re.search('_phi$|phase', xname.lower()) and (sc == 'Freq(MHz)' or re.search('contour',self.plottype)):
                   continue
               if  sc == 'Pwr In(dBm)' and (( test_type == 'linear') or self.plottype in ['contour']) :
                   continue
               if self.logfile_type != 'atr':
                   continue
               if sc not in series_conditions_done:
...
#                     print '(get_selected_plot_data)  xyv list= ',  xyv
                      ...
                      dlst.extend( txyv )
                      rnlst.extend( rnl )
                  elif type(xyv) == types.StringType:
...
#                       print 'X,X2=', X, X2
#                       print 'x,x2=', x, x2
#                       Z = matplotlib.mlab.griddata(x, x2, y,   X,   X2 )
#                   
#                       self.ax.grid(False)
...
#                       self.ax.grid(False)
#                       self.ax.scatter(x, x2,marker=marker,s=5, alpha=0.5)
#                       self.ax.grid(False)
# 
#                       lbl =  'CoNtOuR'
#                       done_a_plot = True
...
#          print '(on_button_press)1',  self.pick_data
          self.key = event.key
          if self.pick_data:
...
#                print '(on_button_press) onpick points:', thisline, ind, self.plot_xdata[ sn ][ ind ], self.plot_ydata[ sn ][ ind ]
...
#          Tk.Checkbutton( tabfrm, name='vramp_search_entry' , variable=self.vramp_search_enable )  .grid( column=3, row=3,sticky=Tk.N)
          ...
          Tk.Entry(  tabfrm, name='script_file_name' , width=20, textvariable=self.script_file_name )                      .grid( row=5, column=1, columnspan=2, sticky=Tk.W+Tk.E)
          ...
          if 0:
              ...
              Tk.Entry( tabfrm, width=20, textvariable=self.vam_offset_entry )        .grid( column=2, row=3, columnspan=2, sticky='w')
              ...
              dist_process_w = Tix.ComboBox(tabfrm, label="Process:", dropdown=1, name='dist_process', editable=1,
                  ...
                  options='listbox.height 5  listbox.width 55 entry.width 25 label.width 10 label.anchor w entry.state normal')
              ...
              dist_freq_w = Tix.ComboBox(tabfrm, label="Freq:", dropdown=1, name='dist_freq', editable=1,
                  ...
                  options='listbox.height 10  listbox.width 55 entry.width 25 label.width 10 label.anchor w entry.state normal')
...
#####################################################################################
    ...
    def mercury_load_db( self, site = None ):
...
#          names   = Tk.Listbox(tabfrm, name='lb', width=40, height=40, yscrollcommand=axis_sb.set, exportselection=0, selectmode=Tk.EXTENDED)
      ...
      scriptlist.column_width(0, chars=15)
...
#      Tk.Button(dbfrm, text="Refresh",      command=self.mercury_search  ) .grid(row=4, column=5)
...
#      self.dbwin.bind("<Double-Button>", self.do_dbwin_event_doublebuttonpress,  )
    ...
    def mercury_get_parts_testers_list( self, site ):
        ...
        cnxn = pyodbc.connect(self.mercury_dsn)
        cursor = cnxn.cursor()
...
#####################################################################################
    ...
    def atlantis_load_db( self, site = None ):
      '''Create a new window for entering the database results
      Parameters:   None
      Returns:      None
      '''
...
#       wgt.grid( column=1, row=1, sticky='w' )
      ...
      Tk.Entry( adbfrm, width=47, textvariable=self.atlantis_desc ).grid( columnspan=2, column=1, row=3, sticky='w')      
      ...
      #shl = Tix.tixTList(dbfrm, name='script_list', options='hlist.columns 5 hlist.header 1 ' )
      #shl = Tix.TList(dbfrm, name='script_list' )
      shl = Tix.ScrolledHList(adbfrm, name='script_list', options='hlist.columns 7 hlist.header 1 ' )
      shl.grid  (columnspan=6, column=0, row=9 )
      scriptlist=shl.hlist
      scriptlist.column_width(2,0)
      scriptlist.config(separator='.', width=150, height=20, drawbranch=0, indent=10,   selectmode=Tk.EXTENDED )
#          names   = Tk.Listbox(tabfrm, name='lb', width=40, height=40, yscrollcommand=axis_sb.set, exportselection=0, selectmode=Tk.EXTENDED)
      ...
      scriptlist.column_width(0, chars=20)
      scriptlist.column_width(1, chars=10)
      scriptlist.column_width(2, chars=45)
      scriptlist.column_width(3, chars=10)
      scriptlist.column_width(4, chars=25)
      scriptlist.column_width(5, chars=20)
      scriptlist.column_width(6, chars=20)
...
#      self.adbwin.bind("<Double-Button>", self.do_adbwin_event_doublebuttonpress,  )
    ...
    def atlantis_get_parts_list( self, site ):
        ...
        cnxn = pyodbc.connect(self.atlantis_dsn)
        cursor = cnxn.cursor()
    ...
    def atlantis_set_dsn(self, site):
        ...
        if re.match(r'San Jose',site): server = 'xxxx';       database = 'xxxx'
...
#        rows = cursor.fetchmany(400) # limit it to 400 rows max
        ...
        for row in rows:
            ...
            t = time.strftime( "  %d%b%y_%H%M", datetime.datetime.timetuple(row.StartTime))
            self.atlantis_session_list.item_create(e, 5, itemtype=Tix.TEXT, text=t.lower())
            self.atlantis_session_list.item_create(e, 6, itemtype=Tix.TEXT, text=row.Station)
            self.atlantis_search_list[ e ] = [  row.Station, row.EventID, row.PartNum, row.SerialNum, row.TestGroup ] 
...
# """
    ...
    ##########################################################
    def atlantis_load_all_sessions(self):
...
#        print '(atlantis_load_test_res) key=', key
        ...
        self.atlantis_load_test(cursor, key, logfilename )
    ...
    ##########################################################
    def  atlantis_load_test(self, cursor, key, logfilename):
        '''Load the results for a single testplan'''
        
        
        search_row = self.atlantis_search_list[key]
        # =[ vcTesterName, biSessionID, vcPartFamily, vcBoardNumber, vctestplanName ] 
        SerialNum = search_row[0]
        EventID   = search_row[1]
        TestDesc  = search_row[2]
        PartNum   = search_row[3]
        TestGroup = search_row[4]
        ...
        # Get list of columns for this testplan
        count = self.atlantis_count_test_rows( cursor, EventID)

        # Get a count of how many test lines there are
        CondColumns, ResultColumns = self.atlantis_get_test_columns( cursor, EventID)
 
        
#        print '(atlantis_load_test)', vcBoardNumber, count, len(columns), search_row
#        print columns
        
        # Add the data
        self.atlantis_add_test_data( cursor,  SerialNum, EventID, \
            TestGroup, CondColumns, ResultColumns, logfilename)

    ##########################################################
    def  atlantis_add_test_data(self, cursor,  SerialNum, EventID, \
            TestGroup, CondColumns, ResultColumns, logfilename):
        '''Add the atlantis test data into pygram self.data'''
        
        
        
        
        print 'CondColumns=', len(CondColumns)
        for c in CondColumns:
            print '    ', c
        print 'ResultsColumns=', len(ResultColumns)
        for c in ResultColumns:
            print '    ', c
        return        
...
#        sql = 'RF9840_SN008|24325|Select %s from  %s Order by DataID' % ( colnames, TableString )
        sql = "Select %s from  %s%s Order by DataID" % ( columns_str, "atlantis_Cache.." , testplan_id )

#        print '(atlantis_add_test_data) sql=', sql
        ...
        # Then add a few bits of static data that were not available in the last query
        self.atlantis_add_misc_columns(rn_start, rn_finish,
              vcTesterName, vcBoardNumber, biSessionID, testplan_id, logfilename )
    ...
    #################################################################################################################
    def atlantis_add_misc_columns( self, rn_start, rn_finish, vcTesterName, vcBoardNumber, biSessionID, testplan_id, logfilename ):
        '''Add misc data columns to self.data'''
    ...
    ############################################################
    def atlantis_count_test_rows( self, cursor, EventID):
        '''Get a count of the number of individual tests for this testplan session'''
    ...
    ############################################################
    def atlantis_get_test_columns( self, cursor, EventID):
        ''' Get a list of column heading names for this testplan session '''
        ...
        return  CondColumns, ResultColumns 
