...
###################################################################
    ...
    def __init__( self ):
      ...
      txt = '$Revision: 1.235 $'                   # sos revision number, auto assigned by sos when doing a checkin
...
#                  print '(add_logfile) vramp data ',self.data['Vramp File'][rn], [ vramp_release, hb_lb, seg, voltage, full_filespec, pcl_level ]
      ...
      if 'Test Freq(MHz)' in self.data and 'Freq(s2p)' not in self.data:
           self.add_new_column('Freq(GHz)')
           for rn in range( rn_start, rn_finish ):
...
#                     self.data['Linear Gain (dB)'][rn] = self.data['Pout(dBm)'][rn] - self.data['Pwr In(dBm)'][rn] 
    ...
    def add_all_power_values( self, rn_start, rn_finish ):
      ...
      if 'Adj Pwr Out(dBm)' in self.data or 'PSA Pwr Out(dBm)' in self.data or 'Pout(dBm)' in self.data:
         ...
         for rn in range( rn_start, rn_finish ):
             pdbm = None
             if    'PSA Pwr Out(dBm)' in self.data and self.data[ 'PSA Pwr Out(dBm)' ][rn] != None:
       ...
       if xname != 'Frequency of Spur (MHz)' and xname[:5] != '[Time]' :
         ...
         for sc in self.value_dict_names:
            if sc != xname and sc in self.series_seperated_list and sc != 'Vramp Voltage':
               ...
               if test_type == 'linear' and sc == 'Pwr In(dBm)' and xname in series_test_list : continue
               if sc not in series_conditions_done:
                   series_conditions_done[ sc ] = 'done'
                   full_series_conditions.append( sc )
...
#                   # Go through all the data series. 
#                   # The data in the series is expected be ascending phase (xpol) 
#                   # then the VSWR (x2pol) will change to a new value.
#                   # When the VSWR (x2pol) changes record the first value,
#                   # and then at the end of the current VSWR (x2pol) value add the 
#                   # first value point with a phase (xpol) that is 2*pi greater than the first point.
#                   
#                   # (or 359.9 to be more precise)
#                   xpl = []
#                   x2pl = []
...
#                   for xp, x2p, yp in zip(xpol,x2pol,ypol):
#                   
#                     # if the VSWR value changes to a new value 
#                     # take the first value of the previous VSWR series
#                     # and add it to the end.
#                     if x2p != x2plz:
#                        xpl.append(xplz  + 2*nu.pi*(359.9/360.0))
#                        x2pl.append(x2plz)
#                        ypl.append(yplz)
#                        # record the first value for the new VSWR series
#                        xplz = xp
#                        x2plz = x2p
#                        yplz = yp
#                        
