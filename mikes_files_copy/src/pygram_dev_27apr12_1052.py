...
###################################################################
    ...
    def __init__( self ):
      ...
      txt = '$Revision: 1.221 $'                   # sos revision number, auto assigned by sos when doing a checkin
...
#                                              'Regmap', 'Process', 'Ref Pout(dBm)'  ]
      ...
      self.xaxis_reduced_list  =  [ 'Freq(MHz)', 'Temp(C)', '[Time] Time',  '[Time] VAM', '[Time] VRAMP', '[Time] SCOPE CHANNEL 4',
                                    'VSWR', 'Phase(degree)', 'Source VSWR', 'Source Phase(degree)',
                                    'Pwr In(dBm)', 'Pout(dBm)', 'VAM(volt)', 'Vbat(Volt)',
...
#               self.values_dict[ vdn ] = []
      ...
      self.scale_data( 'Vbat(A)',     'Vbat_I(mA)' , 1000,  rn_start, rn_finish )
...
#                 print '    %10s  %10s' % (vramp, vramp2pout[ nc_match_name ][vramp])
...
#         self.scale_time_data( '[Time] VBAT_THIRD',   '[Time] VBAT*'    , 3,  rn_start, rn_finish )
    ...
    def add_aclr_data( self,  rn_start, rn_finish ):
      ...
      # add WCDMA data
      if 'Pwr at Center Freq (dBc)' in self.data or 'Pwr at Center Freq (dBm)' in self.data:
          if 'Pwr at -5 MHz Offset (dBc)'   in self.data and \
             ...
             'Pwr at +10 MHz Offset(dBc)'  in self.data    :
              ...
              self.add_new_column( 'Pout(dBm)' )
              self.add_new_column( 'Pwr at Center Freq (dBm)' )
              
              self.add_new_column( 'Pwr at 5MHz Offset(dBc)' )
              self.add_new_column( 'Pwr at 10MHz Offset(dBc)' )
...
#  ACLR10 = -Pwr at Center Freq (dBc)  +  Pwr at 10MHz Offset (dBc)
              ...
              for rn in range( rn_start, rn_finish ):
                if 'Pwr at Center Freq (dBc)' in self.data and self.data['Pwr at Center Freq (dBc)'][rn] != None and self.data['Pwr at Center Freq (dBm)'][rn] == None: 
                   self.data[ 'Pwr at Center Freq (dBm)' ][rn]   = self.data['Pwr at Center Freq (dBc)'][rn]
                if self.data['Pwr at Center Freq (dBm)'][rn] != None and self.data['TestName'][rn] == 'ACLR':
                  self.data['ACLR-5'][rn]  = self.data['Pwr at -5 MHz Offset (dBc)'][rn]  - self.data['Pwr at Center Freq (dBm)'][rn]
                  self.data['ACLR-10'][rn] = self.data['Pwr at -10 MHz Offset (dBc)'][rn] - self.data['Pwr at Center Freq (dBm)'][rn]
                  self.data['ACLR+5'][rn]   = self.data['Pwr at +5 MHz Offset(dBc)'][rn]   - self.data['Pwr at Center Freq (dBm)'][rn]
                  self.data['ACLR+10'][rn]  = self.data['Pwr at +10 MHz Offset(dBc)'][rn]  - self.data['Pwr at Center Freq (dBm)'][rn]
                  self.data['Pwr Gain (PSA)'][rn] = self.data['Pwr at Center Freq (dBm)'][rn] - self.data['Pwr In(dBm)'][rn]
                  self.data[ 'Pout(dBm)' ][rn]    = self.data['Pwr at Center Freq (dBm)'][rn]
                  self.data[ 'Pwr at 5MHz Offset(dBc)' ][rn] = max( self.data[ 'Pwr at +5 MHz Offset(dBc)' ][rn], self.data[ 'Pwr at -5 MHz Offset (dBc)' ][rn] )
                  self.data[ 'Pwr at 10MHz Offset(dBc)' ][rn] = max( self.data[ 'Pwr at +10 MHz Offset(dBc)' ][rn], self.data[ 'Pwr at -10 MHz Offset (dBc)' ][rn] )
#  ACLR-5 = -Pwr at Center Freq (dBc)  +  Pwr at -5MHz Offset (dBc)
       ...
       if xname != 'Frequency of Spur (MHz)' and xname[:5] != '[Time]' :
         ...
         for sc in self.value_dict_names:
            if sc != xname and sc in self.series_seperated_list and sc != 'Vramp Voltage':
               if sc == 'Pwr In(dBm)' and xname in ['Pout(dBm)', 'Ref Pout(dBm)', 'Ref2 Pout(dBm)', 'Pout_pm(dBm)', 'EVM Total Pout over a slot (dBm)' ] : continue
               if sc not in series_conditions_done:
                   series_conditions_done[ sc ] = 'done'
                   full_series_conditions.append( sc )
