...
###################################################################
    ...
    def __init__( self ):
      ...
      txt = '$Revision: 1.246 $'                   # sos revision number, auto assigned by sos when doing a checkin
      txt = re.sub( '.*Revision\S*\s+','X',txt)  # change R_e_v_i_s_i_o_n  to  d_e_v, this will be changed to X and V when released
      txt = re.sub( r'\$','',txt)
...
#                  print '(add_logfile) vramp data ',self.data['Vramp File'][rn], [ vramp_release, hb_lb, seg, voltage, full_filespec, pcl_level ]
      ...
      # If we see Freq(s2p) data then copy it Freq(MHz) overriding the original data.
...
#         if re.search( r'Pout', name , re.I) or re.search( r'VSWR', name , re.I):
       ...
       if xname != 'Frequency of Spur (MHz)' and xname[:5] != '[Time]' :
         ...
         if 'Modulation' in self.data and self.data['Modulation'][first_rn] in ['wcdma','hsupa','hsdpa']:
            test_type = 'linear'
         else:
            test_type = 'saturated'
         ...
         for sc in self.value_dict_names:
            if sc != xname and sc in self.series_seperated_list and sc != 'Vramp Voltage':
               ...
               # in this case we don't want to split the series on pwer in
               series_test_list =  [ 'Pout(dBm)',                           \
                                     'Ref Pout(dBm)',                       \
                                     'Ref2 Pout(dBm)',                      \
                                     'Pout_pm(dBm)',                        \
                                     'PSA Pwr Out(dBm)',                    \
                                     'EVM Total Pout over a slot (dBm)',    \
                                     'EVM Peak(%)',                         \
                                     'EVM RMS(%)',                          \
                                     'EVM Magnitude Error(%)',              \
                                     'EVM Phase Error(%)',                  \
                                     ...
                                     'ACLR +5MHz(dBc)',                     \
                                     'ACLR -5MHz(dBc)',                     \
                                     'neg 8_5MHz Pk Pwr(dBc)',	            \
                                     'neg 7_5MHz Pk Pwr(dBc)',	            \
                                     'neg 3_5MHz Pk Pwr(dBc)',	            \
                                     'neg 2_5MHz Pk Pwr(dBc)',	            \
                                     'pos 2_5MHz Pk Pwr(dBc)',	            \
                                     'pos 3_5MHz Pk Pwr(dBc)',	            \
                                     'pos 7_5MHz Pk Pwr(dBc)',	            \
                                     'pos 8_5MHz Pk Pwr(dBc)',	            \
                                     'Freq(MHz)',                           \
                                   ] 
               # dont split on Freq in a phase plot 
               if re.search('_phi$|phase', xname.lower()) and (sc == 'Freq(MHz)' or re.search('contour',self.plottype)):
                    continue
               if  sc == 'Pwr In(dBm)' and (( test_type == 'linear'  and xname in series_test_list) or self.plottype in ['contour']) : 
                   continue
               if sc not in series_conditions_done:
                   series_conditions_done[ sc ] = 'done'
                   full_series_conditions.append( sc )
...
#                 print sdai, self.data['linenumber'][trn]  , self.data['//T#'][trn], self.data['TestName'][trn],  self.data['Test Freq(MHz)'][trn], sda
                  ...
                  if xlen > ylen:
                     x = x[:ylen]
                     if  plottype in ['polar_contour_plot','contour'] :  x2 = x2[:ylen]
                     rnl = rnl[:ylen]
...
#          Tk.Checkbutton( tabfrm, name='vramp_search_entry' , variable=self.vramp_search_enable )  .grid( column=3, row=3,sticky=Tk.N)
          ...
          Tk.Entry(  tabfrm, name='script_file_name' , width=30, textvariable=self.script_file_name )                      .grid( row=5, column=1, columnspan=2, sticky=Tk.W+Tk.E)


          runscript = Tix.Button(tabfrm, name='script',  text='Run\nScript', width=5, height=2, command=self.run_script)   .grid( column=3, row=5, sticky=Tix.SE)
...
#####################################################################################
    def get_readfiles_dialog(self, message, dirtype, def_filetype=None):
       ...
       if 1:
           ...
           filetypes = [('logfiles', ('*.log','*.s2p')),('logfile', '*.log'), ('csvfile', '*.csv'), ('text', '*.txt'), ('s1p', '*.s1p'), ('s2p', '*.s2p'), ('Excel', '*.xls'), ('all', '*.*')]
...
#####################################################################################
    def wloadfile( self ):
         ...
         filenames = askopenfilename( defaultextension='.log',
                                     ...
                                     multiple=True,
                                     filetypes=[('logfiles', ('*.log','*.s2p')), ('csvfile', '*.csv'), ('text', '*.txt'), ('s1p', '*.s1p'), ('s2p', '*.s2p'), ('all', '*.*')] )
...
#           if re.search( 'filter_value_', ctl_action ) and self.keysym == 'Return':
           if re.search( 'filter_value_', ctl_action ) and click_type == 'key':
