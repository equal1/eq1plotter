...
###################################################################
    ...
    def __init__( self ):
      ...
      txt = '$Revision: 1.268 $'                   # sos revision number, auto assigned by sos when doing a checkin
...
#              self.prd_header_lst = [ '#', 'parameters', 'data', 'upper limit', 'pass/fail', 'comments']
           ...
           if self.xl_type == 'filter_cm':
              row_hdr = 2
              self.prd_header_lst = [ '#', 'parameters', 'meas', 'max', 'min', 'debug' ]
...
#        grps = re.search( r'^\s*(\d+)\s*MHz', prd_test[ 'parameters' ] )
        grps = re.search( r'(\d+)\s*MHz', prd_test[ 'parameters' ] )   # Frequency value can be anywhere in the parameter
        if grps:
            p  = grps.groups()[0].strip()
            cl.append( 'Freq=%s' % p )
...
#            reader = csv.reader(fip, delimiter='\t')
          ...
          try:
              ...
              delim = '\t'
              print "*** warning (load_logfile) cannot determine type of logfile not read file (guessing comma delimitted)" , logfilename
...
#         if re.search( r'Pout', name , re.I) or re.search( r'VSWR', name , re.I):
       ...
       if xname != 'Frequency of Spur (MHz)' and xname[:5] != '[Time]' :
         ...
         if 'Modulation' in self.data and  first_rn in self.data['Modulation'] and \
         ...
         for nam in xynames:
            if len(nam) >= 3 and nam[:3] == 'ACP' or re.search('gain',nam.lower()):
              test_type = 'linear'

         print '@@@@', xynames, test_type
         ...
         for sc in self.value_dict_names:
            if sc not in xynames and sc in self.series_seperated_list and sc != 'Vramp Voltage':
               ...
               if  sc == 'Pwr In(dBm)' and (( test_type == 'linear') or self.plottype in ['contour']) :
                   continue
               if  sc == 'Pwr In(dBm)' and xynames[0] == 'Pout(dBm)' :
                   continue
               if self.logfile_type != 'atr':
                   continue
               if sc not in series_conditions_done:
...
#                 pl[0] = [ [ xmin , lines[0] ], [xmax , lines[0]] , [xmax , lines[1] ], [xmin , lines[1] ] ]
             ...
             self.ax.plot( x , y , color, dashes=dashes, linewidth=thickness, zorder=0)
    ...
    def atlantis_get_sweepdata_table_data(self, cursor, EventID, PartNum):
        ...
        for i,r in enumerate(rows):            
            ...
            if TestName_x not in sweep_data['XValue']:
                sweep_data['XValue'][TestName_x] = []
                sweep_data['YValue'][TestName_y] = []
                sweep_data['ZValue'][TestName_z] = []
            
            sweep_data['XValue'][TestName_x].append(r[3])
            sweep_data['YValue'][TestName_y].append(r[4])
            sweep_data['ZValue'][TestName_z].append(r[5])
            
        if len(rows) > 0:
            sweep_length = len(sweep_data['XValue'][TestName_x])
        else:
            sweep_length = 0
